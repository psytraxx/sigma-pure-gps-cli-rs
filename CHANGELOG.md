...
- `protocol::load_unit_info` now verifies the 76-byte unit-info response before any command
  ...
  checksum, and requires the model identifier at byte 69 to be `0x33` (Pure GPS). This
  ...
  trivially. The model byte is compared as BCD, matching
  `Gps10Decoder.decodeInitialInformation`, which renders the byte as hex text and parses it
  as decimal before comparing against 33 — so the accepted raw byte is `0x33`, verified
  against hardware.
